#ifndef UnitCubeCCCounter_txx
#define UnitCubeCCCounter_txx
...
::operator()() const
{
    unsigned int const neighborhoodSize = TConnectivity::GetInstance().GetNeighborhoodSize();
    unsigned int seed=0;
    // Find first seed
    ...
    while(seed != neighborhoodSize)
    {
        ...
        while(!q.empty())
        {
            ...
            {
                if(!processed[neighbor] && m_Image[neighbor] !=0 && m_UnitCubeNeighbors(current, neighbor))
                {
                    q.push(neighbor);
        ...
        // Look for next seed
        while(seed != neighborhoodSize && ( processed[seed] || m_Image[seed] == 0 || !m_ConnectivityTest[seed] ) )
        {
            ++seed;
        }
    }
...
UnitCubeCCCounter<TConnectivity, TNeighborhoodConnectivity>
::m_UnitCubeNeighbors = UnitCubeNeighbors<TConnectivity, TNeighborhoodConnectivity>();

}

#endif // UnitCubeCCCounter_txx
