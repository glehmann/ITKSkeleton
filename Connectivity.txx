  ...
  m_NeighborsOffsets(new Offset[m_NumberOfNeighbors] )
{
    ...
    for(int i=0; i< std::pow( float(VDim), 3); ++i)
    {
        ...
        {
            {
                Point & neighbor = *(m_NeighborsPoints+currentNbNeighbors);
                neighbor = p;
            }
            {
                Offset & neighbor = *(m_NeighborsOffsets+currentNbNeighbors);
                neighbor = i;
            }
            ++currentNbNeighbors;
        }
...
::AreNeighbors(Point const & p1, Point const & p2) const
{
    ...
    Point* const iterator = std::find(m_NeighborsPoints, m_NeighborsPoints+m_NumberOfNeighbors, difference);
...
{
    Point* const iterator = std::find(m_NeighborsPoints, m_NeighborsPoints+m_NumberOfNeighbors, p);
    return ( iterator != m_NeighborsPoints+m_NumberOfNeighbors );    
}
...
{
    Offset* const iterator = std::find(m_NeighborsOffsets, m_NeighborsOffsets+m_NumberOfNeighbors, o);
